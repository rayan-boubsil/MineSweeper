...
class Game:
    def __init__(self, manager, config):
        ...
        self.audio.start_music("assets/sound_music.ogg")
    ...
    def handle_event(self, event):
        # 1. Gestion des touches système (Pause, Reset, Quitter)
        if event.type == pygame.KEYDOWN:
            # Toggle Pause (P ou Echap)
            if event.key in [pygame.K_ESCAPE, pygame.K_p] and not self.game_over:
                self.audio.play_sfx("click") # Feedback sonore pause
                self.toggle_pause()
                return 
            # Reset (R)
            if event.key == pygame.K_r:
                self.audio.play_sfx("click") # Feedback sonore reset
                self.reset()
                return
            # Quitter ou Menu (Q)
            if event.key == pygame.K_q:
                self.audio.stop_music() # Optionnel : on coupe la musique avant de sortir
        ...
        if self.game_over or self.paused:
            ...
            return 
        # 3. Gameplay (Smiley & Grille)
        mx, my = pygame.mouse.get_pos()
        # Clic sur le Smiley Reset
        if event.type == pygame.MOUSEBUTTONDOWN and self.reset_btn_rect.collidepoint(mx, my):
            self.audio.play_sfx("click") # Feedback sonore au clic sur le bouton
            self.reset()
            return
        # Clics sur la Grille
        if event.type == pygame.MOUSEBUTTONDOWN:
            # Le son est géré à l'intérieur de _process_grid_click et handle_click
    ...
    def handle_click(self, row, col):
        ...
        # On ne joue un son que si la case est cliquable (pas de drapeau, pas déjà révélée)
        if cell.marker_state == 0 and not cell.revealed:
            ...
            # 2. CAS DE VICTOIRE
                ...
                self.end_game(True)
            # 3. CAS DE JEU NORMAL (Case vide ou chiffre)
            else:
                self.audio.play_sfx("click") # <--- Indispensable pour le feedback !
    ...
    def save_current_state(self):
        data = {"config": self.config, "grid": self.grid,"elapsed_before_pause": self.elapsed_time,"won": self.won, "game_over": self.game_over}
